...
let makeSoundVariance (r : rule) (pre : assertions) : proof = 
	...
	if rule_isPredname "subtype" r && term_isConstr (rule_getInputOfConclusion r) && term_isConstr (rule_getOutputOfConclusion r) then 
		let c = term_getCNAME (rule_getInputOfConclusion r) in 
		let argsOnTheLeft = term_getArguments (rule_getInputOfConclusion r) in 
		let argsOnTheRight = term_getArguments (rule_getOutputOfConclusion r) in 
		if not(List.mem (Mutable(c)) pre) then NodeRule(pre, r, pre, "neutral") else 
		let my_check_invariant ((t1,t2)) : bool = 
			(if t1 = t2 || List.mem (Formula("equal", [t1 ; t2])) (rule_getPremises r) 
				then true 
				else let wrongVarianceMsg = (
							if List.mem (Formula("subtype", [t1 ; t2])) (rule_getPremises r) 
							then " as covariant with premise: " ^ print_formula (Formula("subtype", [t1 ; t2])) ^ ". " 
							else 
								if List.mem (Formula("subtype", [t2 ; t1])) (rule_getPremises r) 
								then " as contravariant with premise: " ^ print_formula (Formula("subtype", [t2 ; t1])) ^ ". " 
								else " as not invariant. ") in 
					 		    let _ = track_failed_premise (SoundVariance(c)) (msgSoundVariance r c t1 t2 wrongVarianceMsg) in false)
			in 
		if List.for_all my_check_invariant (List.combine argsOnTheLeft argsOnTheRight)
			then NodeRule(pre, r, SoundVariance(c) :: pre, "SoundVariance")
			else NodeRule(pre, r, pre, "neutral")
	else NodeRule(pre, r, pre, "neutral")
